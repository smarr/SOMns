...
public class StepMessage extends IncommingMessage {
  ...
  public void process(final FrontendConnector connector, final WebSocket conn) {
    ...
    if (susp != null && susp.getEvent() != null) {
      ...
      if (step != SteppingType.RESUME) {
        FrontendConnector.log(
            "[DEBUGGER] Executing " + step.name + " for actor " + activityId);
      }
    } else if (step == SteppingType.RESUME) {
      // notify frontend the actor that can be resumed in the GUI because the actor is not
      // suspended,
      // this is needed for actors that are paused explicitly in the frontend, and they are not
      // actually suspended yet (mailbox is empty)
      ...
      FrontendConnector.log(
          "[DEBUGGER]: Failed to get suspension for activityId: " + activityId);
    }
  ...
  private void sendResumeMessage(FrontendConnector connector) {
    if (step == SteppingType.RESUME
        || step == SteppingType.RETURN_FROM_TURN_TO_PROMISE_RESOLUTION) {
      // send resume message
