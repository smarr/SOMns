...
public class ObjectLiteralNode extends LiteralNode {
   ...
   * (defined by this node's `MixinDefinition`) that has the current activation as its enclosing
   * scope. Once this class has been created, this node will instantiate and return a new object
   * from that class.
   */
  public ObjectLiteralNode(final MixinDefinition mixinDefiniton, final ExpressionNode outerRead,
      final ExpressionNode newMessage, final SourceSection source) {
    super(source);
    this.mixinDef = mixinDefiniton;
    this.superClassResolver = mixinDefiniton.getSuperclassAndMixinResolutionInvokable().createCallTarget();
