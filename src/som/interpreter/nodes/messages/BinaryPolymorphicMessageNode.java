...
public abstract class BinaryPolymorphicMessageNode extends BinaryMessageNode {
  ...
  public BinaryPolymorphicMessageNode(final SSymbol selector,
  ...
  public BinaryPolymorphicMessageNode(final BinaryPolymorphicMessageNode node) {
  ...
  @Override
  @Specialization
  public Object doGeneric(final VirtualFrame frame, final Object rcvr, final Object argument) {
    ...
    for (i = 0; i < cacheEntries; i++) {
      if (rcvrClasses[i] == currentRcvrClass) {
        return invokables[i].invoke(frame.pack(), receiver, new SAbstractObject[] {(SAbstractObject) argument});
      }
    }
    ...
    if (i < CACHE_SIZE) { // we got still room in this polymorphic inline cache
      ...
      cacheEntries++;
      return invkbl.invoke(frame.pack(), receiver, new SAbstractObject[] {(SAbstractObject) argument});
    } else {
