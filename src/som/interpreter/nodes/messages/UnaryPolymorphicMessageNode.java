...
public abstract class UnaryPolymorphicMessageNode extends UnaryMessageNode {
  ...
  public UnaryPolymorphicMessageNode(final SSymbol selector,
  ...
  public UnaryPolymorphicMessageNode(final UnaryPolymorphicMessageNode node) {
  ...
  public Object doGeneric(final VirtualFrame frame, final Object rcvr) {
    ...
    for (i = 0; i < cacheEntries; i++) {
      if (rcvrClasses[i] == currentRcvrClass) {
        return invokables[i].invoke(frame.pack(), receiver, noArgs);
      }
    }
    ...
    if (i < CACHE_SIZE) { // we got still room in this polymorphic inline cache
      ...
      cacheEntries++;
      return invkbl.invoke(frame.pack(), receiver, noArgs);
    } else {
